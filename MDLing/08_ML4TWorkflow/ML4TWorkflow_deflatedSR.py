...
def get_analytical_max_sr(mu, sigma, num_trials):
    ...
    # Euler-Mascheroni constant: limiting difference between the harmonic series and the natural logarithm
    emc = 0.5772156649
...
def main():
    ...
    for i, prod_ in enumerate(product(np.linspace(-100, 100, 101), range(10, 1001, 10)), 1):
        ...
        output.append([mu, sigma, num_trials, n_iter, expected_max_sr, mean_max_sr, std_max_sr, err])
    output = pd.DataFrame(output, columns=['mu', 'sigma', 'num_trials', 'n_iter', 'expected_max_sr', 'mean_max_sr',
                                           'std_max_sr', 'err'])
    print(output.info())
