...
def get_backtest_data(predictions='lasso/predictions'):
    with pd.HDFStore(f'{DATA_DIR}/assets.h5') as store:
        prices = (
            store['quandl/wiki/prices'].filter(like='adj').rename(columns=lambda x: x.replace('adj_', '')).swaplevel(
                axis=0))

    with pd.HDFStore(f'{DATA_DIR}/data.h5') as store:
        print(store.info())
...
print(df.info())
df.to_hdf(f'{DATA_DIR}/backtest.h5', 'data')
